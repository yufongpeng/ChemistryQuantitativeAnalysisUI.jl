...
if !Sys.islinux()
    ...
    UI.run!(initial_mc_r; dir = joinpath(@__DIR__(), "data"), file = "initial_mc_r", newfile = false, async = false, timeout = 60)

    @info "Check the calibration result in $repo2 after saving it within 1 minute"
    UI.run!(repo2; async = true, timeout = 60)
else 
    @test_broken UI.run!(initial_mc_r; dir = joinpath(@__DIR__(), "data"), file = "initial_mc_r", newfile = false, async = false, timeout = 60)
    @test_broken UI.run!(repo2; async = true, timeout = 60)
end
